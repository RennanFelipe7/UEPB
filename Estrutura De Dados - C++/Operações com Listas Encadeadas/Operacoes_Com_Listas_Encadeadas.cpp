...
bool RemoveInicio(noLista *&ApontadorInicioLista){
  noLista *PonteiroPegaPosicaoExcluir = new noLista;
  if(ApontadorInicioLista != NULL){
    PonteiroPegaPosicaoExcluir = ApontadorInicioLista;
...
bool RemoveFim(noLista *&ApontadorInicioLista){ 
  noLista *PonteiroPercorreEExclui = new noLista;
  PonteiroPercorreEExclui = ApontadorInicioLista;
  ...
  if(ApontadorInicioLista == NULL){ 
    ...
    delete[] PonteiroPercorreEExclui;
    return true;
  }else{
    while(PonteiroPercorreEExclui -> prox != NULL){
      PonteiroGuardaPosicaoAnterior = PonteiroPercorreEExclui;
      PonteiroPercorreEExclui = PonteiroPercorreEExclui -> prox;
    }
...
bool RemovePosicao(noLista *& ApontadorInicioLista, int Posicao){
  ...
  if(Posicao > Tamanho){
    ...
    if(Posicao == 1){
      RemoveInicio(ApontadorInicioLista);
    }else{
      int Contador = 1;
      ...
      while(Contador != Posicao){
        ...
        PonteiroPegaPosicaoDelete = PonteiroPegaPosicaoDelete ->prox; 
        PonteiroPegaPosicaoPosteriro = PonteiroPegaPosicaoDelete -> prox;
        Contador++; 
      }
      PonteiroPegaPosicaoAnterior -> prox = PonteiroPegaPosicaoPosteriro;
    ...
    switch (opcao){
      ...
      case 5: 
        if(RemoveInicio(l1)){
          ...
          cout<< "\nLista vazia, impossivel realizar operacao ! \n";
        }
        break;
      case 6:
        if(RemoveFim(l1)){
